  ...
  return (
    <div className="backdrop-blur-md bg-white/20 p-6 rounded-xl shadow-lg border border-white/30">
      ...
      {transactions.length === 0 ? (
        <div className="text-white text-center py-4 backdrop-blur-sm bg-white/10 rounded-lg">
          No records found
        </div>
      ) : (
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <div className="flex flex-col sm:flex-row gap-3 flex-1">
                    ...
                    <input
                      ...
                      className="p-2 rounded-lg bg-white/70 border border-white/30 backdrop-blur-sm w-full sm:w-28 focus:outline-none focus:ring-2 focus:ring-white/50"
                      step="0.01"
                    />
                  </div>
                  <div className="flex gap-2 justify-end sm:ml-3 mt-3 sm:mt-0">
                    <button
                      onClick={() => handleSave(transaction.id)}
                      className="p-2 px-4 text-sm bg-green-500/80 hover:bg-green-600/80 text-white rounded-lg backdrop-blur-sm transition-colors flex-1 sm:flex-none"
                    >
                      Save
                    </button>
                    <button
                      onClick={handleCancel}
                      className="p-2 px-4 text-sm bg-gray-500/80 hover:bg-gray-600/80 text-white rounded-lg backdrop-blur-sm transition-colors flex-1 sm:flex-none"
                    >
                      Cancel
                    </button>
                  </div>
